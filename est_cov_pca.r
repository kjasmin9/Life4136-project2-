...
vcf <- read.vcfR("C:/Users/jaxk2/Downloads/2.vcf") #Filtered and LD-pruned VCF file
...
ggplot(pcs, aes(PC1, PC2, color=as.factor(ploidy)))+
    ...
    labs(x=xlab, y=ylab, color="Ploidy", 
         title = "PCA analysis with tetraploid and diploid populations")+
    geom_text_repel(aes(label=id), size=3.28, force=30, color="black", max.overlaps = 100, na.rm = TRUE)+
    theme(panel.background = element_rect(fill = '#e9ddef'),
          panel.grid.major=element_blank(), 
        panel.grid.minor=element_blank(), 
        panel.border=element_blank(),
        axis.line=element_line(color="#e9ddef",size=1),
